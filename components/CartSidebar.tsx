...
export default function CartSidebar({ user }: CartProps) {
    ...
    return (
        <aside className={`fixed overflow-y-hidden right-0 top-0 w-[200px] bg-[#fff2f2] h-full shadow-lg z-50 p-4 ${showCart && cart.length > 0 ? "block" : "hidden"} ${url.startsWith('/productos') || url == '/' ? "block" : "hidden"}`}>
